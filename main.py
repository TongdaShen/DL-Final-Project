...
relative_weight2 = relative_weight[0]

def style_content_loss_more_color(outputs):
      ...
      style3_loss = tf.add_n([tf.reduce_mean((style_outputs[name]-style3_targets[name])**2) 
                            for name in style_outputs.keys()])
  ...
  for x in range(5):
    for y in range(5):
      ...
      if sum >= 3*region_height*region_width*0.5 or sum <= 3*region_height*region_width*0.2:
        ...
        print(str(x)+str(y))
        print(sum)
        print(3*region_height*region_width*0.5)
